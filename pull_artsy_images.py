...
def main():
    for artwork in artsy_db.get_artworks():
        art_id = artwork["__id"]
        artwork_file = os.path.normpath(
            f"{os.path.dirname(os.path.realpath(__file__))}/data/{art_id}_normalized.jpg")
        if not os.path.exists(artwork_file):
            res = requests.get(
                artwork["_links"]["image"]["href"].format(image_version="normalized"))
